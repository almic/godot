...
class WheelSettings : public Resource {
	GDCLASS(WheelSettings, Resource);
...
public:
	...
	// TODO: merge these into a single helper method like 'configure_wheel_angles(camber, toe, caster, kingpin)'
	//       do not store the parameters, do not provide a method to return the inputs
...
VARIANT_ENUM_CAST(WheelSettings::SpringMode);

class WheelSettingsWV : public WheelSettings {
	GDCLASS(WheelSettingsWV, WheelSettings);
...
protected:
	...
	void _apply_lateral_friction() {
		to_jolt(*lateral_friction.ptr(), settings.mLateralFriction);
	}

	void _apply_longitudinal_friction() {
		to_jolt(*longitudinal_friction.ptr(), settings.mLongitudinalFriction);
	}
};