...
public:
	...
	virtual void		AddHit(const ShapeCastResult &inResult) override
	{
		...
		if (!found)
		{
			...
			if (shape_index < mContactPointOffset.size())
				current_point_offset = mContactPointOffset[shape_index];
			else
				current_point_offset = mContactPoint.size();
			// Offset increment happens later, when the point is inserted
			mContactPointOffset.insert(mContactPointOffset.cbegin() + shape_index, current_point_offset);
		}
	...
	void				FinalizeContacts()
	{
		...
		Array<Vec3> average_normal_list;
		average_normal_list.resize(mContactBody.size());
			...
			if (point_index == next_shape_at_point)
			{
				...
				if (shape_index == next_body_at_shape)
				{
					if (body_point_count > 0)
					{
						...
						average_normal_list[body_index] = average_contact_normal.NormalizedOr(mUpDirection);
		...
		if (mPickCollisionsFunction != nullptr)
			kept_body_list = mPickCollisionsFunction(mPickCollisionsContext, mContactBody, mFractionList, mContactPointAverage, average_normal_list);
		else
			...
			if (point_index == next_shape_at_point)
			{
				...
				if (shape_index == next_body_at_shape)
				{
					if (body_point_count > 0)
					{
						...
						// NOTE: mUpDirection is inverted, so it points the same direction as normals
		...
		mContactNormal.resize(body_keep_index);
	}
	...
	float				mClosestFraction;		///< After finalize: Closest hit fraction
...
bool SpringCastCollisionTesterCastSphere::Collide(PhysicsSystem &inPhysicsSystem, RVec3Arg inOrigin, Vec3Arg inDirection, SpringCastConstraint &ioSpringCastConstraint) const
{
	...
	for (size_t body_index = 0; body_index < num_bodies; ++body_index)
	{
		...
		AxisConstraintPart &spring_part = ioSpringCastConstraint.mSpringPart[body_index * 2];
		AxisConstraintPart &spring_hard_part = ioSpringCastConstraint.mSpringPart[body_index * 2 + 1];
		SpringCastConstraint::FrictionPart &friction1 = ioSpringCastConstraint.mFrictionConstraint[body_index * 2];
		SpringCastConstraint::FrictionPart &friction2 = ioSpringCastConstraint.mFrictionConstraint[body_index * 2 + 1];
		SpringCastConstraint::AngularPart &angular_friction = ioSpringCastConstraint.mAngularConstraint[body_index];

		// These are uninitialized normally, so we must ensure they are cleared before use.
		// Otherwise, infinities, nans, or just large lambdas can be waiting
		...
		for (size_t other_index = 0; other_index < old_num_bodies; ++other_index)
		{
			...
			spring_part.SetTotalLambda(old_spring_part[other_index * 2].GetTotalLambda());
			spring_hard_part.SetTotalLambda(old_spring_part[other_index * 2 + 1].GetTotalLambda());
			friction1.SetTotalLambda(old_friction_part[other_index * 2].GetTotalLambda());
			friction2.SetTotalLambda(old_friction_part[other_index * 2 + 1].GetTotalLambda());
			angular_friction.SetTotalLambda(old_angular_part[other_index].GetTotalLambda());
...
void SpringCastConstraint::OnStep(const PhysicsStepListenerContext &inContext)
{
	...
	mPlaneConstant.resize(mNumContactBody);
...
void SpringCastConstraint::SetupVelocityConstraint(float inDeltaTime)
{
	...
	const float spring_c = mSpringLength - (mSettings->mMaxLength + mSpringRestOffset);
...
bool SpringCastConstraint::SolveVelocityConstraint(float inDeltaTime)
{
	...
	for (uint sorted_index = 0; sorted_index < mContactBodyIDSorted.size(); ++sorted_index)
	{
		...
		if (linear_friction_active)
		{
			...
			// Reduce impulse by up to mForwardMaxSpeed
			if (has_forward && lambda1 > 0.0f)
				lambda1 -= min(mForwardMaxSpeed * friction1.GetEffectiveMass(), lambda1);
...
Vec3 SpringCastConstraint::GetFrictionForce(uint inBodyIndex) const {
	...
	if (friction1.IsActive())
		friction_force += t1 * friction1.GetTotalLambda();

	if (friction2.IsActive())
		friction_force += t2 * friction2.GetTotalLambda();
...
void SpringCastConstraint::SaveState(StateRecorder &inStream) const
{
	...
	inStream.Write(mSpringRestOffset);
...
void SpringCastConstraint::RestoreState(StateRecorder &inStream)
{
	...
	inStream.Read(mSpringRestOffset);
