...
public:
	...
	float					mMaxLength = 0.5f;				///< How long the spring is at max extension relative to the attachment point (m), this is the rest length of the spring
...
public:
	...
	/// Virtual destructor
	virtual							~SpringCastCollisionTester() = default;
...
public:
	...
	virtual EConstraintSubType	GetSubType() const override					{ return EConstraintSubType::User1; }
	...
	/// Returns the contact position in world space
	RVec3						GetContactPoint(uint inContactPointIndex) const { JPH_ASSERT(inContactPointIndex < mContactPoint.size()); return mContactPoint[inContactPointIndex]; }

	/// Returns the contact position in world space of the body
	RVec3						GetContactPoint(uint inContactPointIndex, uint inBodyIndex) const { uint contact_point_index = CheckContactPointIndex(inBodyIndex, inContactPointIndex); return mContactPoint[contact_point_index]; }

	/// Returns the contact position in world space for a specific shape of the body
	RVec3						GetContactPoint(uint inContactPointIndex, uint inBodyIndex, uint inSubShapeIndex) const { uint contact_point_index = CheckContactPointIndex(inBodyIndex, inSubShapeIndex, inContactPointIndex); return mContactPoint[contact_point_index]; }

	/// Returns the total number of contact points
	uint						GetNumContactPoint() const { return mContactPoint.size(); }
	...
	RVec3						GetContactPointAverage(uint inBodyIndex) const { CheckBodyIndex(inBodyIndex); return mContactPointAverage[inBodyIndex]; }
	...
	/// Get the rest offset of the spring (m)
	float						GetSpringRestOffset() const					{ return mSpringRestOffset; }

	/// Set the rest offset of the spring (m), which is relative to the max length
	void						SetSpringRestOffset(float inSpringRestOffset) { mSpringRestOffset = inSpringRestOffset; }
	...
	/// Get total directional force (N s) applied as planar friction per body
	Vec3						GetFrictionForce(uint inBodyIndex) const;
